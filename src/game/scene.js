...
function makeFloorReflectionSprite(hex) {
...
function makeSaber(hex) {
  ...
  // Dev hitbox. Trzymamy go kompletnie ukrytego, dopóki panel ?dev go nie włączy.
  // Samo opacity=0 bywało zdradliwe przy przezroczystościach/depth bufferze.
  ...
  g.userData = { bladeGlow: bgMat2, outerGlow: ogMat, shineMat: shMat, shineMesh: shMesh, shine2Mat: sh2Mat, shine2Mesh: sh2Mesh, color: hex, wireMat, wireMesh };
  return g;
}
...
/**
 * Zmienia kolor miecza w czasie rzeczywistym.
 * @param {'left'|'right'} side
 * @param {string} hex
 */
export function setSaberColor(side, hex) {
  ...
  if (saber.userData.bladeGlow) {
    saber.userData.bladeGlow.color.set(color);
  }
  if (saber.userData.outerGlow) {
    saber.userData.outerGlow.color.set(color);
  }
  ...
  if (reflection?.material) {
    ...
    reflection.material.map = makeGlowTexture(colorHex);
    reflection.material.map.needsUpdate = true;
    reflection.material.needsUpdate = true;
  }

  saber.traverse(child => {
    if (child.isMesh && child.material?.emissive !== undefined && child.material.emissiveIntensity > 0) {
  ...
  saber.userData.color = colorHex;
}

export function animateIdleSabers(t) {
...
export function updateLightReflections(t) {
  ...
  for (const s of [lSaber, rSaber]) {
    ...
    s.userData.shineMesh.position.y  = 0.25 + phase * 0.82;
    s.userData.shine2Mesh.position.y = 0.55 + ((phase + 0.36) % 1) * 0.48;
    s.userData.shineMat.opacity  = glint * 0.22;
    s.userData.shine2Mat.opacity = glint * 0.14;
  }
}
...
export function setWireframeVisible(visible) {
  ...
  for (const saber of [lSaber, rSaber]) {
    const mesh = saber.userData.wireMesh;
    const mat  = saber.userData.wireMat;
    if (!mesh || !mat) continue;
    mat.color.set(0xffffff);
    mat.opacity = on ? 0.42 : 0;
    mat.visible = on;
    mesh.visible = on;
    mesh.scale.setScalar(on ? 1 : 0.0001);
...
export function triggerShake(intensity = 0.06) {
  shakeIntensity = Math.max(shakeIntensity, intensity);
}

export function applyShake(deltaScale = 1) {
...
const QUALITY_CHECK_MS = 1500; // sprawdzaj co 1.5 sekundy niezależnie od fps
let _lastResizeAt = 0;
let autoSlowChecks = 0;
let autoFastChecks = 0;
let lastAutoTierChangeAt = 0;

function publishGraphicsStatus() {
...
function applyDecorVisibility() {
  ...
  for (const s of [lSaber, rSaber]) {
    if (!perfProfile.saberGlints) {
      s.userData.shineMat.opacity = 0;
      s.userData.shine2Mat.opacity = 0;
    }
  }
}

function applyActiveProfileDpr(targetDpr = null) {
  applyDecorVisibility();
  renderer.toneMappingExposure = ['lowest', 'very-low', 'low'].includes(perfProfile.qualityMode) ? 1.0 : 1.2;
  const deviceDpr = window.devicePixelRatio || 1;
  const nextDpr = clampDpr(Number.isFinite(targetDpr) ? targetDpr : Math.min(deviceDpr, perfProfile.maxDpr), perfProfile);
...
export function setScenePerformanceProfile(settingsOrProfile = {}) {
  perfProfile = settingsOrProfile.camera ? { ...settingsOrProfile } : getPerformanceProfile(settingsOrProfile);
  autoSlowChecks = 0;
  autoFastChecks = 0;
...
function setAutoGraphicsTier(qualityMode, now) {
  const nextProfile = getPerformanceProfile({ performanceMode: 'auto', autoQualityMode: qualityMode });
...
export function adaptRenderQuality(frameMs, fps = 0) {
