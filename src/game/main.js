...
import { initAudio, startMapAudio, stopMapAudio, pauseMapAudio, getMapTime, getMapDuration, setVolume, setMusicVolume, setSfxVolume, setSoundVolume, applyAudioSettings, loadMapAudio, hasMapAudio, clearMapAudio } from './audio.js';
...
function preserveDevQueryOnMenuLinks() {
  const current = new URLSearchParams(location.search);
...
// ── ?map= URL param ───────────────────────────────────────────────────────────
async function tryLoadMapFromUrl() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (validateMap(map)) {
        state.map = { ...map, _serverAudioPending: true };
        return;
      }
    }
  } catch {}
  ...
  if (validateMap(localMap)) {
    state.map = { ...localMap, _localAudioPending: true, localOnly: true };
  }
}

async function ensureCurrentMapAudio() {
...
function startMapTimeline(now = performance.now()) {
  mapTimelineZeroAtMs = now + MAP_LEAD_IN_MS;
  mapAudioStarted = false;
  pausedMapTimelineSec = null;
}

function getMapTimelineSec(now = performance.now()) {
...
function getCurrentMapDuration() {
...
function updateMapAudioSchedule(now = performance.now()) {
...
function pauseMapTimeline(now = performance.now()) {
  if (!state.map) return;
  pausedMapTimelineSec = getMapTimelineSec(now);
  if (hasMapAudio() && mapAudioStarted) pauseMapAudio();
}

function resumeMapTimeline(now = performance.now()) {
...
function pauseGame(reason, now = performance.now()) {
...
function updateHandsPauseState(now) {
  ...
  } else if (state.appState === S.PAUSED && canAutoResumeFromHands(state.pauseReason)) {
    if (ready) {
...
function frameScaledLerp(baseAmount, deltaScale) {
  return 1 - Math.pow(1 - baseAmount, Math.max(0, deltaScale));
}

function applyTrackedSaberQuaternion(saber, trackedQuat, amount = 0.14) {
...
function updateMenuAutoplay(now, t) {
  updateMenuDemo(now, t);
  const target = window.__menuDemoTarget;

  const idleLeft = {
    x: -0.72 + Math.sin(t * 1.35) * 0.22,
    y: 1.08 + Math.sin(t * 1.75) * 0.26,
    z: 1.55,
  };
  const idleRight = {
    x: 0.72 + Math.sin(t * 1.35 + 1.6) * 0.22,
    y: 1.08 + Math.sin(t * 1.65 + 0.8) * 0.26,
    z: 1.55,
  };
...
let renderMs = 0;
let detectMs = 0;
let mainLoopRaf = null;
let mainLoopRunning = false;
let _nearestBeat = null;
...
function loop(timestamp) {
  ...
  const previousNow = Number.isFinite(loop._lastNow) ? loop._lastNow : now - BASE_FRAME_MS;
  ...
  if (isMainMenuOpen()) {
    ...
    lSaber.userData.bladeGlow.opacity = pulse;
    rSaber.userData.bladeGlow.opacity = pulse;
    ...
    animateIdleSabers(t);
    lSaber.userData.bladeGlow.opacity = 0.7 + Math.sin(t * 4) * 0.15;
    rSaber.userData.bladeGlow.opacity = 0.7 + Math.sin(t * 4 + 1) * 0.15;
  }
  ...
  // Odczyt renderer.info tuż po render() — wartości są aktualne tylko w tej chwili
...
function initMapDrop() {
  ...
  canvas.addEventListener('drop', async e => {
    ...
    try {
      ...
      state.map = validateMap(map) ? map : { ...map, beats: null };
      ...
      if (ui.dStatus) ui.dStatus.textContent = `MAP ERROR: ${err.message}`;
    }
  });
}
...
async function startFromMainMenu({ calibrate = false } = {}) {
  initAudio();
  applyAudioSettings(settings);
  setScenePerformanceProfile(settings);
  prewarmGameplayResources();
...
function initMainMenu() {
  ...
  const navItems = [...document.querySelectorAll('.main-nav-item:not(.is-disabled)')];
  ...
  const volumeInput = document.getElementById('menuVolume');
  const soundInputs = [...document.querySelectorAll('[data-audio-setting]')];
  const noFailInput = document.getElementById('menuNoFail');
  const beatLimitInput = document.getElementById('menuBeatLimit');
  const flipCameraInput = document.getElementById('menuFlipCamera');
  const performanceInput = document.getElementById('menuPerformanceMode');
  ...
  const developerModeInput = document.getElementById('menuDeveloperMode');
  ...
  const audioOffsetInput = document.getElementById('menuAudioOffset');
  const audioOffsetValue = document.getElementById('menuAudioOffsetValue');
  const oneHandButtons = [...document.querySelectorAll('[data-one-hand]')];
  ...
  function setSettingsPanelVisible(visible) {
    ...
    if (visible) {
      requestAnimationFrame(() => settingsPanel?.querySelector('input,button,summary')?.focus?.());
    } else {
      settingsButton?.focus?.({ preventScroll:true });
    }
  }

  function syncOneHandButtons() {
  ...
  function updateSettingsSliderValue(input) {
    if (!input?.id) return;
    const valueEl = document.querySelector(`.settings-slider-value[data-for="${input.id}"]`);
    if (!valueEl) return;
    const min = Number(input.min || 0);
    const max = Number(input.max || 100);
    const value = Number(input.value || 0);
  ...
  function bindStyledRange(input) {
    if (!input) return;
    const update = () => {
  ...
  function getGraphicsModeSummary() {
  ...
  function updateGraphicsModeInfo() {
  ...
  for (const item of navItems) {
    ...
    item.addEventListener('click', () => {
      ...
      window.clearTimeout(item._sliceTimer);
      item.classList.remove('slicing');
      void item.offsetWidth;
      item.classList.add('slicing');
      item._sliceTimer = window.setTimeout(() => item.classList.remove('slicing'), 400);
    });
  }
  ...
  for (const input of soundInputs) {
    ...
    input.value = String(settings[key] ?? 1);
  ...
  if (performanceInput) {
    const updatePerformanceHint = () => {
      ...
      const mode = getPerformanceMode({ performanceMode: performanceInput.value });
  ...
  for (const btn of oneHandButtons) {
    btn.addEventListener('click', () => {
      const value = btn.dataset.oneHand || null;
  ...
  // ── Kolory mieczy ─────────────────────────────────────
  function updateColorPreview(previewBar, previewName, colorDef) {
    if (previewBar) {
  ...
  function buildColorGrid(gridEl, previewBar, previewName, side, currentHex) {
    if (!gridEl) return;
    ...
    for (const colorDef of SABER_COLORS) {
      ...
      btn.addEventListener('click', () => {
        ...
        settings[settingKey] = colorDef.hex;
        setSetting(settingKey, colorDef.hex);
