...
export function applyBackgroundTheme(palette: {
  base: string; top: string;
  nebulaA: string; nebulaB: string;
  cloud: string; horizon: string;
  star: string; lane: string; accent: string;
}): void {
  const set = (name: string, hex: string): void => {
    const u = bgMat.uniforms[name];
    if (u && u.value instanceof THREE.Color) (u.value as THREE.Color).set(hex);
  };
  set('uBase',    palette.base);
  set('uTop',     palette.top);
  set('uNebulaA', palette.nebulaA);
  set('uNebulaB', palette.nebulaB);
  set('uCloud',   palette.cloud);
  set('uHorizon', palette.horizon);
  set('uStar',    palette.star);
  set('uLane',    palette.lane);
  set('uAccent',  palette.accent);
