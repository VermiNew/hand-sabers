...
const MAX_EQUALIZER_BAR_COUNT = 16;
...
const equalizerGeometry = new THREE.BoxGeometry(0.07, 1, 0.07);
const equalizerMaterial = new THREE.MeshBasicMaterial({
  color: THEME.white,
  transparent: true,
  opacity: 0,
  depthWrite: false,
...
const equalizerBars = new THREE.InstancedMesh(equalizerGeometry, equalizerMaterial, MAX_EQUALIZER_BAR_COUNT);
...
equalizerBars.count = 0;
equalizerBars.visible = false;
equalizerBars.frustumCulled = false;
equalizerBars.renderOrder = 0;
equalizerBars.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
scene.add(equalizerBars);
...
function syncPortalColors(): void {
  ...
  for (let index = 0; index < MAX_EQUALIZER_BAR_COUNT; index++) {
    portalColor.setHex(index % 2 === 0 ? left : right);
    equalizerBars.setColorAt(index, portalColor);
  }
  ...
  if (equalizerBars.instanceColor) equalizerBars.instanceColor.needsUpdate = true;
}
...
const EQUALIZER_BAR_COUNTS: Record<string, number> = {
  lowest: 0,
  'very-low': 0,
  low: 0,
  medium: 6,
...
export function updateMusicVisualizer(frame: MusicVisualizerFrame): void {
  ...
  const equalizerBarCount = frame.profile.qualityMode === 'custom'
    ? Math.max(0, Math.min(MAX_EQUALIZER_BAR_COUNT, Math.round(frame.profile.arenaDetail * 12.8)))
    : EQUALIZER_BAR_COUNTS[frame.profile.qualityMode] ?? 6;
  ...
  equalizerBars.count = equalizerBarCount;
  equalizerBars.visible = equalizerBarCount > 0 && intensity > 0.001;
  equalizerMaterial.opacity = Math.min(0.34, (0.055 + levels.overall * 0.16 + beatPulse * 0.12) * intensity) * readability;
  ...
  for (let index = 0; index < equalizerBarCount; index++) {
    const pairIndex = Math.floor(index / 2);
    const side = index % 2 === 0 ? -1 : 1;
    const band = pairIndex % 3 === 0 ? levels.bass : pairIndex % 3 === 1 ? levels.mid : levels.treble;
    const ripple = 0.5 + Math.sin(frame.nowSec * 3.1 - pairIndex * 0.82) * 0.5;
    const height = 0.16 + (band * 1.15 + beatPulse * 0.32 * ripple) * intensity * readability;
    portalTransform.position.set(side * (2.8 + pairIndex * 0.055), height * 0.5, -5.4 - pairIndex * 1.72);
    portalTransform.scale.set(1, Math.max(0.08, height), 1);
    portalTransform.rotation.set(0, 0, side * -0.035);
    portalTransform.updateMatrix();
    equalizerBars.setMatrixAt(index, portalTransform.matrix);
  }
  if (equalizerBarCount > 0) equalizerBars.instanceMatrix.needsUpdate = true;
}
