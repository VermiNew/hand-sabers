...
6. [x] Ograniczyć zwykłą konsolę do najważniejszych logów i dodać rozszerzony tryb developerski odblokowywany sekretem skonfigurowanym po stronie serwera oraz zgodną wartością podaną w ustawieniach klienta. — token 12–256 znaków pochodzi z ignorowanego przez Git `config.local.json` lub — z wyższym priorytetem — `HAND_SABERS_DEVELOPER_TOKEN`; repozytorium zawiera bezpieczny `config.local.example.json`. Endpoint stosuje limit 10 prób/min, stałoczasowe porównanie skrótów i `no-store`, nie zwracając sekretu. Ustawienia mają pole `password` i czytelne stany brak konfiguracji/błędny token/dostęp; wartość jest czyszczona po próbie, nie trafia do bundla, URL, storage ani eksportu i po odświeżeniu wymaga ponownej autoryzacji. Zapisane `developerMode`, `?dev`, `?testing`, ręczny event oraz debug hit-plane nie omijają bramki, ale po odblokowaniu zachowują dotychczasowe funkcje. Zwykły klient nie ma `console.log/info/debug`, kontrolowane fallbacki używają warningów tylko po autoryzacji, a istotne `console.error` pozostają widoczne. Smoke konfiguracji plikowej potwierdził stan aktywny, odrzucenie błędnej wartości i przyjęcie właściwej; README opisuje oba źródła konfiguracji PL/EN.
...
12. [x] Przeprojektować panel wyboru map, nadając mu bardziej charakterystyczny, dopracowany kierunek inspirowany jakością interfejsów gier takich jak *Overwatch*, bez kopiowania ich zasobów ani układu 1:1. — picker otrzymał własny kierunek „biblioteki aren”: szerszą kompozycję, numerowane okładki generowane proceduralnie z ID i trudności mapy, licznik wyników oraz rozbudowane hero wybranej ścieżki z identyfikacją koloru i wizualizatorem rytmu. Zachowano wyszukiwanie, filtry, ulubione, sortowanie, import, rekomendacje, preview, AUTO, grę i „Rzuć wyzwaniem”; nie użyto obcych assetów ani skopiowanego layoutu. Smoke Playwright potwierdził focus/close, 23 mapy, działający wybór i brak błędów, a inspekcja 1440 px i 390 px doprowadziła do poprawki licznika i ograniczenia bardzo długich tytułów bez utraty pełnej nazwy w tooltipie.
...
14. [x] Zaprojektować od nowa wskaźnik HP i wskaźnik ukończenia mapy, czerpiąc ogólne inspiracje czytelnością HUD-ów *Doom Eternal*, *Overwatch*, *Beat Saber* i *Lockdown Protocol*. — dolny moduł otrzymał własną, zwartą hierarchię „combat vitals”: podpis Integralność z ikoną tarczy, licznik HP i dziesięć czytelnych segmentów, zachowując istniejące stany pełny/średni/krytyczny, animację straty/zysku i ostrzeżenie ekranowe. Postęp pokazuje jednocześnie procent, pozostały czas, segmentowany tor i punkt pozycji. Oba mierniki mają aktualizowane `role=progressbar`, minimum, maksimum i wartość dla technologii asystujących. Smoke prawdziwej mapy AUTO potwierdził aktualizację czasu/procentu bez błędów, a inspekcja 1440×900 potwierdziła czytelność przy aktywnych kostkach i mieczach bez zasłaniania toru.
15. [ ] Ulepszyć UX/UI edytora map, aby praca z nim była przyjemniejsza i mniej przypominała generyczny program narzędziowy.
16. [x] Poprawić płynność postrzeganego renderowania kostek; zbadać między innymi głębię sceny, mgłę i inne subtelne sposoby ograniczenia wrażenia skokowego ruchu. — znaleziono rzeczywistą przyczynę: `updateBlocks()` kończyło pracę przed aktualizacją pozycji przy częstotliwości wyższej niż 60 Hz, więc na ekranie 120 Hz kostki powtarzały położenie przez dwie klatki. Tani ruch, obrót i synchronizacja macierzy instancji działają teraz z częstotliwością renderera, podczas gdy spawn, kolizje, punktacja i publikacja statystyk pozostają ograniczone do 60 Hz. Zachowano istniejącą mgłę zależną od profilu i głębię areny zamiast dodawać mocniejszą mgłę maskującą błąd. Smoke mapy Maximum potwierdził 9 aktywnych obiektów, rosnący wynik/postęp i brak błędów strony.
...
18. [x] Przeprojektować panel diagnostyki kamery pod kątem czytelności, użyteczności i spójności wizualnej. — panel otrzymał nadrzędną ocenę gotowości wyliczaną z faktycznych pomiarów rozdzielczości, FPS, oświetlenia i uprawnień (dla telefonu bez nieadekwatnego pomiaru światła), jednoznaczne stany pomiar/gotowe/ostrzeżenie/błąd oraz wizualnie sprzężone karty parametrów. Zachowano istniejące progi i źródła śledzenia. Kontrolowany test kamery osiągnął stan gotowości przy 1280×720, 30 FPS i prawidłowym świetle; inspekcja 1440 px i 390 px potwierdziła czytelny układ bez overflow i błędów strony.
...
23. [x] Dodać poziomy/tryby trudności gry: Newbie, Normal, High i Extreme, z jednoznacznym wpływem na rozgrywkę. — cztery istniejące, trwałe i synchronizowane w Multiplayerze poziomy prędkości otrzymały wymagane nazwy oraz jawny mnożnik/czas dolotu: Newbie `0,75× / 2,40 s`, Normal `1,00× / 1,80 s`, High `1,35× / 1,33 s`, Extreme `1,75× / 1,03 s`. Zmieniają czas reakcji bez modyfikowania `beat.t`, układu mapy lub timingu muzyki. Wybór Extreme zapisuje `1.75` i ma poprawne `aria-pressed`; inspekcja desktop/390 px potwierdziła czytelny układ 2×2 bez overflow i błędów strony.
24. [x] Poprawić tryb przestrzenny tak, aby pozycje pozostawały naturalne dla zasięgu dłoni i stabilności ML oraz nie generowały kostek trudnych do wykrycia lub trafienia na środku kadru. — generator zachowuje teraz pusty korytarz centralny `|x| < 0,32` i trzyma cele lewej/prawej ręki po właściwej stronie, także w gęstych formacjach, podczas prób separacji oraz w rzadkim fallbacku po wyczerpaniu prób. Pozostawiono losowość wysokości, rozstaw, deterministyczność multiplayerową i istniejący limit zasięgu kolejnego ruchu. Próba obciążeniowa 300 gęstymi blokami potwierdziła 0 naruszeń centrum/wysokości i identyczny layout w dwóch przebiegach; prawdziwa mapa AUTO uruchomiła tryb przestrzenny z 11 aktywnymi blokami bez błędów strony.
