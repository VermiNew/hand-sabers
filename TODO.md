...
- [ ] Dodać barierę gotowości przed rundą Multiplayer dla mapy, trackingu i audio wszystkich graczy.
  - [ ] Serwer ma rozpocząć rundę dopiero po potwierdzeniu gotowości wymaganych zasobów przez wszystkie aktywne sesje; rozłączenie lub zmiana mapy unieważnia poprzednią gotowość.
    - [x] Pokazać ekran „Sesje graczy ładują się… proszę czekać”, spinner, progressbar `gotowi/wszyscy`, stan każdego gracza i przycisk „Opuść lobby”. — karta oczekiwania korzysta z serwerowego pola `ready`, pojawia się już podczas lokalnego przygotowania, współdzieli istniejącą listę stanów graczy i rozłącza przez tę samą ścieżkę co główny przycisk opuszczenia pokoju.
