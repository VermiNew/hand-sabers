...
- [ ] Zastąpić porównywanie `Origin` z requestowym `Host` jawną listą dozwolonych originów/hostów używaną wspólnie przez REST, `/ws` i `/tracking-ws`; nie włączać przy tym automatycznie `security: true` ani nie usuwać lokalnego trybu developerskiego. — **średni priorytet audytu**: obecny wspólny, kontrolowany przez request punkt odniesienia dopuszcza DNS rebinding do usług przeznaczonych dla zaufanego LAN.
- [ ] Ograniczyć stan rate limiterów po przekroczeniu progu: odrzucone requesty i upgrade'y nie mogą dalej powiększać tablic historii; dodać stały limit per klucz oraz wygaszanie wpisów `/ws`. — **średni priorytet audytu**: obecne filtrowanie rosnącej tablicy daje koszt kwadratowy podczas burstu.
- [ ] Dodać globalny limit bajtów i liczby trwałych map/audio, liczony atomowo przy tworzeniu, zastępowaniu i rollbacku. — **średni priorytet audytu**: quota 256 MB chroni wyłącznie `.uploads`, a zaakceptowane pliki w `beatdata`/`audio` nie mają limitu całkowitego. Domyślną wartość trzeba dobrać do realnej biblioteki; nie zastępuje to osobnego, zablokowanego zadania quota per-user.
- [ ] Nie trzymać blokady mapy przez cały transfer audio/ZIP do klienta; wykonać bezpieczny snapshot/otwarcie pliku pod krótką blokadą, dodać deadline odpowiedzi i nie obejmować globalną blokadą katalogu oczekiwania na blokadę mapy. — **średni priorytet audytu**: wolny odbiorca może zablokować mapę, a równoległe wyszukiwanie po tytule rozszerzyć blokadę na katalog.
- [ ] Usunąć fallback `express.static(PROJECT_ROOT)` albo zastąpić go dedykowanym katalogiem publicznym; brak `dist` ma kończyć się czytelnym błędem zamiast denylistą ścieżek. — **niski priorytet audytu, tylko tryb developerski**: zakodowany separator omija `BLOCKED_STATIC_RE`; zwykłe `npm start` serwuje `dist` i nie jest tą ścieżką.
- [ ] Pozwolić na mikrofon z własnego originu w `Permissions-Policy`, gdy `security: true`, aby opcjonalny czat głosowy nie był blokowany przez nagłówki wdrożeniowe; pozostawić wymaganie jawnego kliknięcia i zgody przeglądarki.
- [ ] Dodać proxy `/tracking-ws` do konfiguracji Vite albo jednoznacznie kierować developerski remote tracking na port Express 3000; obecne `npm run dev` proxy obejmuje `/api` i `/ws`, ale nie kanał telefonu.
